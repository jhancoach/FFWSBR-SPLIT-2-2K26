...
# Add map sort options
sort_logic_old = """        results.sort((a, b) => {
...
sort_logic_new = """        results.sort((a, b) => {
            if (sortBy === 'global_kpm') return b.globalKpm - a.globalKpm;
            if (sortBy === 'total_kills') return b.totalKills - a.totalKills;
            if (sortBy === 's1_kpm') return b.kpmBySafe[1] - a.kpmBySafe[1];
            if (sortBy === 's2_kpm') return b.kpmBySafe[2] - a.kpmBySafe[2];
            ...
            if (sortBy === 's6_kpm') return b.kpmBySafe[6] - a.kpmBySafe[6];
            if (sortBy === 's7_kpm') return b.s7_8_kpm - a.s7_8_kpm;
            if (sortBy === 'ber_kpm') return b.kpmByMap['BER'] - a.kpmByMap['BER'];
            if (sortBy === 'pur_kpm') return b.kpmByMap['PUR'] - a.kpmByMap['PUR'];
            if (sortBy === 'kal_kpm') return b.kpmByMap['KAL'] - a.kpmByMap['KAL'];
            if (sortBy === 'nt_kpm') return b.kpmByMap['NT'] - a.kpmByMap['NT'];
            if (sortBy === 'sol_kpm') return b.kpmByMap['SOL'] - a.kpmByMap['SOL'];
            return b.globalKpm - a.globalKpm;
        });"""
content = content.replace(sort_logic_old, sort_logic_new)

# Modify the Top Controls sort options
top_controls_old = """                        <select 
                            value={sortBy}
...
table_section_old = """                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
...
table_section_new = """                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
                    <h3 className="text-white font-black text-sm uppercase italic tracking-widest flex items-center gap-2">
                        <Activity size={20} className="text-emerald-500" />
...
if table_section_old in content:
    content = content.replace(table_section_old, table_section_new)
else:
    print("Table section old not found!")
