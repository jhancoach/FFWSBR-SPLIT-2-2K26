...
const badCode = `                  <span className="text-white text-xl font-black uppercase flex items-center justify-center w-full h-full">{player.name.substring(0,3)}</span>    ) : activeTab === 'team_perf' && bannerTeamPerfData ? (
...
const goodCode = `                  <span className="text-white text-xl font-black uppercase flex items-center justify-center w-full h-full">{player.name.substring(0,3)}</span>
                )}
                {player.teamImg && (
